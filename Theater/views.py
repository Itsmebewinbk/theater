...
from Theater.forms import TheatreRegistrationForm,TheatreEditForm,ScreenRegistrationForm,ScreenEditForm
...
def list_theatre(request,*args,**kwargs):
    all_theatre=Theater.objects.get(owner=request.user)
...
def add_screen(request):
    ...
    if request.method=="POST":
        form=ScreenRegistrationForm(request.POST,files=request.FILES)
        if form.is_valid():
            form.instance.user=request.user
            form.save()
